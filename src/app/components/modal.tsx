...
export function ModalMembers({ title, roomId, onClose }: ModalMembers) {
...
export function ModalApproveMembers({ title, roomId, onClose }: ModalMembers) {
  const { members,isLoading } = useApproveMembers({ roomId });
  const [container, setContainer] = React.useState<HTMLElement | null>(null);
  const [formValues, setFormValues] = useState({
    code: "",
  });
